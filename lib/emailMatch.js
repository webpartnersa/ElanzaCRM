...
Return ONLY a JSON object: { "is_work_related": boolean, "reason": string (one short sentence), "key_points": string[] (a short bullet-point summary of the email's concrete facts/requests/updates - max 6 points, each a short standalone sentence naming the specific thing stated, e.g. "Fit approved for PL001" or "PO delivery date moved to 2026-10-05" - skip greetings/signatures/boilerplate; empty array if there's genuinely nothing concrete to summarize) }.

If a point states that something specific was confirmed/approved/completed but the email gives no explicit date for it, include this email's own received date in that point (e.g. "Fit approved for PL001 (${'{{receivedDate}}'})") - this email was received on {{receivedDate}}. Only do this for the specific thing actually confirmed, not for unrelated dates.
...
async function classifyWorkRelated(row, openaiClient) {
  if (!openaiClient) return { is_work_related: null, reason: 'OPENAI_API_KEY not configured', key_points: [] };
  const body = (row.text_body || stripHtml(row.html_body || '')).slice(0, 4000);
  const receivedDate = (row.received_at || '').slice(0, 10) || 'unknown';
  const prompt = CLASSIFY_PROMPT
    .replace('{{from}}', row.from_email || '')
    .replace('{{subject}}', row.subject || '')
    .replace(/\{\{receivedDate\}\}/g, receivedDate)
    .replace('{{body}}', body);
  ...
  try {
    ...
    return {
      is_work_related: !!data.is_work_related,
      reason: data.reason || null,
      key_points: Array.isArray(data.key_points) ? data.key_points.filter(p => typeof p === 'string' && p.trim()) : [],
    };
  } catch (e) {
    return { is_work_related: null, reason: 'Could not parse classification response', key_points: [] };
  }
}
...
async function classifyAndMatch(resendEmailId, openaiClient) {
  ...
  const { is_work_related, key_points } = await classifyWorkRelated(row, openaiClient);
  ...
  db.prepare(`
    UPDATE inbound_emails SET
      ...
      match_confidence = ?, match_candidates_json = ?, key_points_json = ?, classified_at = CURRENT_TIMESTAMP
    WHERE resend_email_id = ?
  `).run(
    is_work_related === null ? null : (is_work_related ? 1 : 0),
    result.status, result.type, result.id, result.confidence,
    JSON.stringify(result.candidates), JSON.stringify(key_points || []), resendEmailId
  );
  return result;
}
