...
function recordLabel(matchType, matchId) {
  ...
  if (matchType === 'order') {
    // Same fallback lib/emailMatch.js's findByCode already uses for order
    // candidates - many real orders have no order_no yet (a PO number
    // only exists once the buyer actually issues one), so order_no alone
    // would show as a bare "null" for a perfectly real, in-progress order.
    const row = db.prepare('SELECT id, order_no, style_no, description FROM orders WHERE id = ?').get(matchId);
    if (!row) return null;
    return { id: row.id, no: row.order_no || row.style_no, description: row.description };
  }
  const noCol = matchType === 'style' ? 'style_no' : 'concept_no';
...
function getLinkedRecords(inboundEmailId) {
  const fromChanges = db.prepare(`
    SELECT DISTINCT match_type, match_id FROM inbound_email_field_changes WHERE inbound_email_id = ?
  `).all(inboundEmailId);
  // The primary match (inbound_emails.match_type/match_id, set on first
  // link - see resolveMatch) must always be included even when it has zero
  // proposed changes of its own - a record with nothing to propose is
  // still linked, not un-linked, and dropping it here previously made the
  // UI silently forget it as soon as a DIFFERENT linked record got any
  // field_changes rows (e.g. a style match with nothing to propose,
  // linked alongside an order match that did have a change).
  const email = db.prepare('SELECT match_type, match_id FROM inbound_emails WHERE id = ?').get(inboundEmailId);
  const all = [...fromChanges];
  if (email && email.match_type && !all.some(r => r.match_type === email.match_type && r.match_id === email.match_id)) {
    all.unshift({ match_type: email.match_type, match_id: email.match_id });
  }
  return all;
}
