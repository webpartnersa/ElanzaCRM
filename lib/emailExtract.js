...
- Special case for milestone/approval DATE fields (any field whose name mentions a "Date" or an event like "Fit"/"Approved"): if the email confirms that milestone happened (e.g. "fit was approved", "fabric approved") but states no explicit date, propose this email's own received date (${receivedDate ? receivedDate : 'not available - skip the field in this case'}) as the value, and set source_snippet to the sentence that confirms the event happened (not a date, since none was stated).
